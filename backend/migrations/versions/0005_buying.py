"""Module 04 — Buying: Request for Quotation, Supplier Quotation,
Purchase Order (with taxes); cycle links onto purchase receipt / purchase
invoice rows.

Revision ID: 0005_buying
Revises: 0004_stock
...
revision: str = "0005_buying"
down_revision: Union[str, None] = "0004_stock"
...
RLS_TABLES = ("requests_for_quotation", "supplier_quotations", "purchase_orders")
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        *_doc_columns(), _company_col(), *_voucher_columns(),
        ...
        sa.Column("message_for_supplier", sa.Text()),
        sa.Column("status", sa.String(30), nullable=False, server_default=sa.text("'Draft'")),
        sa.UniqueConstraint("company_id", "name", name="uq_rfq_name"),
    )
    op.create_table(
        "request_for_quotation_items",
        *_doc_columns(),
        sa.Column("rfq_id", pg.UUID(as_uuid=True),
                  sa.ForeignKey("requests_for_quotation.id", ondelete="CASCADE"), nullable=False),
        sa.Column("idx", sa.Integer(), nullable=False, server_default=sa.text("0")),
    ...
    op.create_table(
        ...
        *_doc_columns(),
        ...
        sa.Column("quote_status", sa.String(30), nullable=False, server_default=sa.text("'Pending'")),
    )
    ...
    op.create_table(
        ...
        *_doc_columns(), _company_col(), *_voucher_columns(), *_totals_columns(),
        sa.Column("supplier_id", pg.UUID(as_uuid=True), sa.ForeignKey("suppliers.id"), nullable=False),
        sa.Column("rfq_id", pg.UUID(as_uuid=True), sa.ForeignKey("requests_for_quotation.id")),
        sa.Column("valid_till", sa.Date()),
        sa.Column("status", sa.String(30), nullable=False, server_default=sa.text("'Draft'")),
        sa.UniqueConstraint("company_id", "name", name="uq_supplier_quotation_name"),
    )
    op.create_table(
        ...
        *_doc_columns(), *_item_row_columns(),
        ...
        sa.Column("rfq_item_id", pg.UUID(as_uuid=True), sa.ForeignKey("request_for_quotation_items.id")),
    )
    ...
    op.create_table(
        ...
        *_doc_columns(), _company_col(), *_voucher_columns(), *_totals_columns(),
        ...
        sa.Column("supplier_quotation_id", pg.UUID(as_uuid=True), sa.ForeignKey("supplier_quotations.id")),
        sa.Column("status", sa.String(30), nullable=False, server_default=sa.text("'Draft'")),
        sa.Column("per_received", sa.Numeric(8, 3), nullable=False, server_default=sa.text("0")),
        sa.Column("per_billed", sa.Numeric(8, 3), nullable=False, server_default=sa.text("0")),
        sa.UniqueConstraint("company_id", "name", name="uq_purchase_order_name"),
    )
    ...
    op.create_table(
        ...
        *_doc_columns(), *_item_row_columns(),
        ...
        sa.Column("warehouse_id", pg.UUID(as_uuid=True), sa.ForeignKey("warehouses.id")),
        sa.Column("schedule_date", sa.Date()),
        _amount("received_qty"),
        _amount("billed_amt"),
        sa.Column("material_request_item_id", pg.UUID(as_uuid=True),
    ...
    # cycle links created without FK in 0004 / 0002 — wire them up now
    op.create_foreign_key(
        "fk_pri_po_item", "purchase_receipt_items", "purchase_order_items",
        ["purchase_order_item_id"], ["id"],
    )
    op.add_column("purchase_invoice_items", sa.Column("purchase_order_item_id", pg.UUID(as_uuid=True)))
    op.create_foreign_key(
        "fk_pii_po_item", "purchase_invoice_items", "purchase_order_items",
        ["purchase_order_item_id"], ["id"],
    )

    for table in ("requests_for_quotation", "supplier_quotations", "purchase_orders"):
        op.execute(
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_pii_po_item", "purchase_invoice_items", type_="foreignkey")
    op.drop_column("purchase_invoice_items", "purchase_order_item_id")
    op.drop_constraint("fk_pri_po_item", "purchase_receipt_items", type_="foreignkey")
    ...
    op.execute("UPDATE purchase_receipt_items SET purchase_order_item_id = NULL")
    for table in (
        "purchase_order_taxes", "purchase_order_items", "purchase_orders",
        "supplier_quotation_items", "supplier_quotations",
        "request_for_quotation_suppliers", "request_for_quotation_items", "requests_for_quotation",
    ):
        op.drop_table(table)
