...
// ERPNext-style Selling workspace: its own module sidebar + content
// (trend chart + number cards + grouped "Reports & Masters" grid).
// Self-contained full-page layout (a top-level route, not under AppShell).
...
import { SELLING_CARDS, SELLING_SIDEBAR } from "@/config/sellingWorkspace";
...
interface Stats {
  ...
  sales_order_trend: { label: string; value: number }[];
}
...
onMounted(async () => {
  try {
    stats.value = (await api.get<Stats>("/selling/workspace")).data;
  } catch {
    stats.value = null;
  } finally {
    loading.value = false;
  ...
  <div class="flex min-h-screen bg-gray-50">
    ...
    <aside class="flex w-60 flex-shrink-0 flex-col border-r border-gray-200 bg-white">
      <div class="flex items-center gap-3 border-b border-gray-200 px-4 py-4">
        ...
        <div>
          <div class="text-sm font-semibold text-gray-900">Selling</div>
          <div class="text-xs text-gray-500">{{ brand.product_name }}</div>
        </div>
      </div>
      <nav class="flex-1 overflow-y-auto p-3">
        <div v-for="(group, gi) in SELLING_SIDEBAR" :key="gi" class="mb-2">
          <div
            v-if="group.title"
    ...
    <main class="flex-1 overflow-y-auto p-6">
      <h1 class="mb-4 text-xl font-semibold text-gray-900">Selling</h1>

      <!-- trend chart -->
      <div class="mb-4 rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
        <h2 class="mb-3 text-sm font-semibold text-gray-700">Sales Order Trends</h2>
        <p v-if="loading" class="py-12 text-center text-sm text-gray-400">Loading…</p>
        <TrendChart v-else-if="stats" :points="stats.sales_order_trend" :height="220" />
      ...
      <div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          <div class="text-sm text-gray-500">Sales Orders</div>
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div class="mt-1 text-3xl font-semibold text-gray-900">
            {{ formatCurrency(stats?.total_sales_amount ?? 0, stats?.currency ?? "INR") }}
          </div>
        </div>
        <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          <div class="text-sm text-gray-500">Average Order Value</div>
          <div class="mt-1 text-3xl font-semibold text-gray-900">
            {{ formatCurrency(stats?.average_order_value ?? 0, stats?.currency ?? "INR") }}
          </div>
        </div>
      </div>
