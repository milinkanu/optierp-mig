...
onMounted(async () => {
  await Promise.all([accounts.fetchSuppliers(), stock.fetchItems(), stock.fetchWarehouses()]);
});
</script>
  ...
  <form @submit.prevent="save">
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <nav class="flex flex-wrap items-center gap-2 text-sm text-gray-500">
        <span>Buying</span><span class="text-gray-300">/</span>
        <span>Purchase Receipt</span><span class="text-gray-300">/</span>
        <span class="font-semibold text-gray-900">New Purchase Receipt</span>
